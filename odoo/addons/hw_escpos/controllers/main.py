...
class EscposProxy(hw_proxy.Proxy):
    ...
    @http.route('/hw_proxy/print_xml_receipt_______________________________', type='json', auth='none', cors='*')
    def print_xml_receipt(self, receipt):
        ...
        for line in receipt.split('++++++')[1:-1]:
            y = 0
            str = ''
            for line_key in line.split('@@@@@'):
                if y != 1:
                   str +=  ''.join(line_key)
                y += 1
            if line.split('@@@@@')[1] in val:
                val[line.split('@@@@@')[1]] += str
            else:
                val[line.split('@@@@@')[1]] = str
        if val:
            for key,value in val.items():
                ...
                with EscposIO(Network(key, port=9100),autocut=False, autoclose=True) as p:
                    ...
                    p.printer.receipt(receipt.split('++++++')[0] + value + receipt.split('++++++')[-1])
        else:
            # with EscposIO(Network('192.168.123.222', port=9100),autocut=False, autoclose=True) as p:
            #     p.printer.text('\x20\x13\x32\x33')
            #     # p.printer.barcode
            #     p.printer.cut()
            # SIZE 40 mm,30 mm
            # GAP 2 mm,0 mm
            ...
            # BAR 30,16,280,5
            # BAR 30,180,280,5
            # TEXT 30,40 ,"1",0,2,2,"NO:4872"
            # TEXT 240,40,"TSS24.BF2",0,1,1,"打印机"
            # BARCODE 30,100,"128M",40,1,0,2,1,"69123888"
            # PRINT 1
            with EscposIO(Network(config.pos_printer, port=9100),autocut=False, autoclose=True) as p:
                #driver.push_task('xml_receipt',receipt)
                p.printer.receipt(receipt)
